...
- **`png_load_mem`:** после обнуления локального фрейма через `rep stosb` в `rdi` оказывался конец буфера, а не входной `buf` — аргументы копируются в `r12`/`r13` **до** `sub rsp` и цикла обнуления.
